...
class Chat extends List {
   constructor(container, stopMessage) {
      super("chat");
      ...
      [
         this,
         form,
         new Comment(`* Please, say "${stopMessage}", for stop this Bot.`),
      ]
...
class Bot extends Chat {
   constructor(container, stopMessage) {
      ...
      this._listner = setInterval(async () => {
         try { 
            const msg = await this.listenChat();

            await Bot.wait(Bot.randomFromRange(1000, 3000));
            super.addPost(msg, typePost.bot);
   ...
   addPost(message, style) {
      super.addPost(message, style);
      this._postsForAnswer.push(message);
   }

   answerPost() {

      if (this._stopBot) return this._stopMsg;
...
const myChatBot = new Bot(container, "Bye");